...
function getDependenciesStream(workingDir) {
  const childProc = childProcess.spawn('npm',
    ['ls', '--production=true', '--parseable=true', '--silent'],
    { cwd: workingDir }
  );
  let result = [];
...
function getDependenciesFile(workingDir) {
  return new BbPromise((resolve) => {
    const depFile = './deps.txt';

    childProcess.exec(
      'npm ls --production=true --parseable=true --silent > deps.txt',
      { cwd: workingDir },
      () => {
        const result = fs.readFileSync(depFile, { encoding: 'utf8' })
          .split('\n')
          .filter((item) => item.length > 0);
...
function excludeNodeDevDependencies(servicePath) {
  ...
  try {
    ...
    return BbPromise.map(packageJsonPaths, (packageJsonPath) => {
      ...
      return getDependenciesFile(dirWithPackageJson);
      // return getDependenciesStream(dirWithPackageJson);
    }, { concurrency: 3 })
    .then((results) => {
